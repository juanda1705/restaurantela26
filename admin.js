...
async function cargarDashboardReal() {
    try {
        // payment_method se lee directamente desde la columna de la BD
        ...
        if (tbodyFacturas) {
            ...
            if (ordenesValidas.length === 0) {
                ...
                // Reconstruir totales por método desde la columna payment_method de la BD
...
async function registrarMetodoPago(orderId, metodo) {
    if (!metodo) return;
    try {
        // 1. Guardar payment_method directamente en la columna de la BD
        const { error } = await supabaseClient
            .from('orders')
            .update({ payment_method: metodo, status: 'paid' })
            .eq('id', orderId);
        if (error) throw error;
        ...
        // 2. Recargar dashboard completo para reflejar el cambio en contadores y tabla
